...
export const showNotification = (
	...
	try {
		const title = notification.title || response?.statusText || capitalizeWord(notification.variant);
		const message =
			notification.desc ||
			(notification.variant == NotificationVariant.SUCCESS ? result.message : result.error) ||
			null;

		notifications.show({
			id: linkify(`${notification.variant}-${response?.status || "500"}-${message}`),
