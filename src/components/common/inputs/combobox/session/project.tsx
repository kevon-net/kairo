...
export default function Project({
  ...
  return (
    <Combobox
      ...
      withinPortal={false}
      onOptionSubmit={(val) => {
        if (props.formSession) {
          props.formSession.setFieldValue('properties.category_id', val);
        }
    ...
    >
      <ComboboxTarget>
        <InputBase
          ...
          rightSection={
            props.formSession?.values.properties.category_id ? (
              <CloseButton
                size="sm"
                onMouseDown={(event) => event.preventDefault()}
                onClick={() => {
                  if (props.formSession) {
                    props.formSession.setFieldValue(
          ...
          rightSectionPointerEvents={
            !props.formSession?.values.properties.category_id ? 'none' : 'all'
          }
        >
          {props.formSession?.values.properties.category_id ? (
            <Group gap={'xs'} fz={'sm'} pr={'xs'}>
              {props.formSession?.values.properties.category_id == 'inbox' ? (
