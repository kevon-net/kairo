...
export const useFormBlogReply = (params: { commentId?: string; replyCommentId?: string }) => {
	...
	const handleSubmit = async () => {
		if (form.isValid()) {
			try {
				...
				if (params.commentId) {
					response = await replyCommentCreate({ ...parseValues(), commentId: params.commentId });
				}

				if (params.replyCommentId) {
					response = await replyReplyCreate({ ...parseValues(), replyCommentId: params.replyCommentId });
				}
