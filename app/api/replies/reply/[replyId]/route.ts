...
export async function POST(request: NextRequest) {
	try {
		const reply: Omit<ReplyReplyCreate, "id" | "replyComment"> & { replyCommentId: string; userId?: string } =
			await request.json();
		...
		const createReply = await prisma.replyReply.create({
			data: { id: generateId(), name: reply.name, content: reply.content, replyCommentId: reply.replyCommentId },
		});
...
export async function PUT(request: NextRequest, { params }: { params: { replyId: string } }) {
	try {
		const replyRecord = await prisma.replyReply.findUnique({ where: { id: params.replyId } });
		...
		await prisma.replyReply.update({ where: { id: params.replyId }, data: reply });
