...
import { Anchor, Avatar, Card, Divider, Grid, GridCol, Group, Stack, Text, Title } from "@mantine/core";
import React, { useState } from "react";
import FormBlogReply from "@/components/form/blog/reply";
import CardBlodReplyReply from "./reply";

export default function Comment({ props }: { props: ReplyCommentGet & { replies?: ReplyReplyGet[] } }) {
	...
	return (
		<Card bg={"transparent"} padding={0} py={"md"}>
			<Stack>
				...
				<Group>
					<Anchor fw={"bold"} underline="always" onClick={() => setMounted(!mounted)}>
						{mounted ? "Close" : "Reply"}
					</Anchor>
				</Group>
						...
						{props.replies.map((reply) => (
							<GridCol key={reply.id} span={12}>
								<Stack gap={0}>
									...
									{props.replies?.indexOf(reply) != props.replies?.length! - 1 && <Divider />}
								</Stack>
							</GridCol>
						))}
					</Grid>
